...
public class MotosikletController : ControllerBase
{
    private readonly IConfiguration _configuration;

    public MotosikletController(IConfiguration configuration)
    {
        _configuration = configuration;
    }

    [HttpPost("teklif")]
    public IActionResult GetTeklif([FromBody] MotosikletFormModel form)
    {
        try
        {
            using var connection = new MySqlConnection(_configuration.GetConnectionString("DefaultConnection"));
            using var command = new MySqlCommand("sp_GetMotosikletTeklif", connection);
